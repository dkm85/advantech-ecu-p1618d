...
extern struct uart_port_ext uart_ext[64];
...
#ifndef PORT_XR17V35X
	#define PORT_XR17V35X 16
#endif
...
static int  serialexar_setRS485(struct uart_port *port, int turnOn, int turnaroundTime);
static unsigned int serialexar_calc_divisor(struct uart_port *port, 
 													unsigned int baud, 
                                           			unsigned int *quot, 
...
//	serial_out(up, UART_DLM, value >> 8 & 0xff);
	...
	if(uart_ext[up->port.line].reuse & 0x01){
		/* code from windows
		if (DesiredBaud == 50)
		{
		...
		*/
		...
		serial_out(up, UART_DLL, value & 0xff);		/* LS of divisor */
		serial_out(up, UART_DLM, value >> 8);		/* MS of divisor */
	}
	/* Shengjia Add End */
}
...
static void serial8250_set_sleep(struct uart_8250_port *p, int sleep)
{
	...
	if (p->port.type == PORT_XR17V25X)
	{
		if (sleep) {
			/* sleep */
			serial_out(p, XR_17V25X_EXTENDED_EFR, UART_EFR_ECB);
			serial_out(p, UART_IER, UART_IERX_SLEEP);
			serial_out(p, XR_17V25X_EXTENDED_EFR, 0);
	...
	else if(p->port.type == PORT_XR17V35X)
	{
		oldEFR = serial_in(p, XR_17V35X_EXTENDED_EFR);
		if (sleep) {
			/* sleep */
			serial_out(p, XR_17V35X_EXTENDED_EFR, oldEFR | 0x10);
			serial_out(p, UART_IER, UART_IERX_SLEEP);
			serial_out(p, XR_17V35X_EXTENDED_EFR, oldEFR &(~0x10));
		} else {
			/* wake */
...
static int serial_get_mode(struct uart_8250_port *up)
{
	...
	up->index = uart_ext[up->port.line].idx;
...
serial8250_rx_chars(struct uart_8250_port *up, unsigned char lsr)
{
	...
	if (up->port.type == PORT_XR17V35X)
	{
		...
		//DEBUG_INTR("icount.rx = %d\n",up->port.icount.rx);
	}
	
   else if( uart_ext[up->port.line].reuse & 0x02)
   {
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 9, 0)
	insertcount = tty_insert_flip_string(&port->state->port, (unsigned char *)up->rx_ring, count);
#else
	insertcount = tty_insert_flip_string(tty, (unsigned char *)up->rx_ring, count);
#endif
...
void serial8250_tx_chars(struct uart_8250_port *up)
{
	...
	if (up->port.type == PORT_XR17V25X || up->port.type == PORT_XR17V35X)
	{
		...
		if(count > 0 && port->type == PORT_XR17V35X)
		{
		...
		else
		{
			...
			{	
				...
				{			
					...
					memcpy_toio(up->port.membase + UART_17V25X_TX_OFFSET, &(xmit->buf[xmit->tail]), tmp);
					xmit->tail += tmp;
					xmit->tail &= (UART_XMIT_SIZE-1);
				...
				{
					memcpy_toio(up->port.membase + UART_17V25X_TX_OFFSET, &(xmit->buf[xmit->tail]), count);	
					xmit->tail += count;
					xmit->tail &= UART_XMIT_SIZE - 1;
...
static irqreturn_t serial8250_interrupt(int irq, void *dev_id)
{
	...
	do {
		...
		if (serial8250_default_handle_irq(port)) {
			handled = 1;
			end = NULL;
		} else if (end == NULL)
			end = l;
...
static int serial8250_startup(struct uart_port *port)
{
	...
	if (port->type == PORT_16C950) {
		...
		//up->acr = 0;
		up->acr = uart_ext[up->port.line].serial_type; // Default ACR value for auto DTR RS485 or RS232
		...
		serial_port_out(port, UART_EFR, UART_EFR_ECB);
		serial_port_out(port, UART_LCR, 0);
		/*Shengjia Modify to minsh togger level for RHR*/
		//serial_icr_write(up, UART_ACR, up->acr);
	...
	else if (port->type == PORT_XR17V25X)
	{
		...
		if ( serial_get_mode(up) == SERIAL_MODE_RS232 )
		{
			serial_out(up, XR_17V25X_EXTENDED_FCTR, XR_17V25X_FCTR_TRGD | XR_17V25X_FCTR_RTS_8DELAY);
		}
		else // 422/485
	...
	else if(port->type == PORT_XR17V35X)
	{
	...
	if (!(port->flags & UPF_BUGGY_UART) &&
		...
		printk_ratelimited(KERN_INFO "ttyAP%d: LSR safety check engaged!\n",
				   serial_index(port));
		return -ENODEV;
	}
	...
	if (port->type == PORT_16850) {
		...
		if (uart_ext[up->port.line].serial_type == ACR_DTR_RS232)	
			serial_out(up, UART_FCTR, serial_in(up, UART_FCTR) & ~UART_FCTR_RS485);
	...
	if(uart_ext[up->port.line].reuse & 0x02){
...
#else	 
		if (skip_txen_test || up->port.flags & UPQ_NO_TXEN_TEST)
#endif		
...
static unsigned int serialexar_get_divisor(struct uart_port *port, unsigned int baud, unsigned int *quot, unsigned int *quot_fraction, unsigned int *devide1)
{

	serialexar_calc_divisor(port, baud, quot, quot_fraction, devide1);

	return 0;  
}

static void serial8250_set_special_baudrate(struct uart_port *port, unsigned int special_baudrate)
{
	...
	port_index = up->port.line;
	quot_fraction = 0;
	printk(KERN_INFO "Enter in serial8250_set non-standard baudrate:%d\n",special_baudrate);
	...
	serialexar_get_divisor(port, baud, &quot, &quot_fraction, &devide1);
	...
	//Fractional baud rate support
	{
	    ...
	    DEBUG_INTR(KERN_INFO "serial8250_set_special_baudrate: quot =0x%x quot_fraction=0x%x DLD_reg=0x%x\n",quot,quot_fraction,reg_read);		
...
static unsigned int serialexar_calc_divisor(struct uart_port *port, unsigned int baud, unsigned int *quot, unsigned int *quot_fraction, unsigned int *devide1)
{
...
//		else
	...
	if(up->port.type == PORT_XR17V25X )
	{
		...
		serial_outp(up, XR17V25X_UART_DLD, quot_fraction & 0xf);
	}
	...
	if(up->port.type == PORT_XR17V35X)
	{
		...
		// set RS485
		(serial_get_mode( up ) == SERIAL_MODE_RS485ORRS422S) ? serialexar_setRS485(&up->port, 1, 0) : serialexar_setRS485(&up->port, 0, 0);
	...
	switch (cmd)
	{
		case ADV_READ_REG:
			...
			if (copy_to_user((void *)arg, &ioctlrwarg, sizeof(ioctlrwarg)))
				return -EFAULT;
			DEBUG_INTR(KERN_INFO "serial8250_ioctl read reg[0x%02x]=0x%02x \n",ioctlrwarg.reg,ioctlrwarg.regvalue);
			ret = 0;
			break;
		...
		case ADV_WRITE_REG:
			...
			DEBUG_INTR(KERN_INFO "serial8250_ioctl write reg[0x%02x]=0x%02x \n",ioctlrwarg.reg,ioctlrwarg.regvalue);
			ret = 0;
			break;
		...
		case ADV_SET_TURN_AROUND_DELAY:
			...
			serialexar_setRS485(&up->port, 1, iArg);
			ret = 0;
			break;
	}
	      ...
	      unsigned int oldstate)
{
	if (port->pm)
		...
		adv_serial8250_do_pm(port, state, oldstate);
}
...
static int serial8250_request_std_resource(struct uart_8250_port *up)
{
	...
	case UPIO_MEM:
		...
		if (port->flags & UPF_IOREMAP) {
			...
			port->membase = ioremap_cache(port->mapbase, size);
#else
...
serial8250_verify_port(struct uart_port *port, struct serial_struct *ser)
{
	...
	if( ser->type == PORT_XR17V35X && ser->custom_divisor > 0)
		serial8250_set_special_baudrate(port,ser->baud_base/ser->custom_divisor);
	return 0;
}
...
int adv_serial8250_register_8250_port(struct uart_8250_port *up)
{
...
int adv_serial8250_register_port(struct uart_port *port)
{
	...
	return adv_serial8250_register_8250_port(&up);
}
...
void __exit adv_serial8250_exit(void)
{
	...
	for (i = 0; i < UART_NR; i++)
	{
		...
		{
			if(serial8250_ports[i].port.type != PORT_XR17V35X)
...
EXPORT_SYMBOL(adv_serial8250_register_port);
EXPORT_SYMBOL(adv_serial8250_do_pm);
EXPORT_SYMBOL(adv_serial8250_do_set_termios);
