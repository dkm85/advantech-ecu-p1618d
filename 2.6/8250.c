 ...
 */
/*static*/ const struct serial_uart_config uart_config[PORT_MAX_8250+3] = {         
	{ "unknown",	1,	0 },
	{ "8250",	1,	0 },
	{ "16450",	1,	0 },
	{ "16550",	1,	0 },
...
static _INLINE_ void transmit_chars(struct uart_8250_port *up)
{
	...
	else
	{
		...
		count = up->port.fifosize >> 1;// /*xuji:for low baudrate*/ count =  1;
		//lipeng modify end
		//james dai add end
		do {
			serial_out(up, UART_TX, xmit->buf[xmit->tail]);
...
static _INLINE_ void transmit_chars(struct uart_8250_port *up)
{
	...
	if (up->port.type == PORT_XR17V35X)
	{
			...
			// read the fifo count untill we get the same value twice
			while (bytes_in_fifo != serial_inp(up, XR_17V35X_TXFIFO_CNT))
				bytes_in_fifo = serial_inp(up, XR_17V35X_TXFIFO_CNT);
		
			//DEBUG_INTR("byte_in_fifo = %d,fifosize = %d\n",bytes_in_fifo,up->port.fifosize);
			// how much buffer is availabe now to write?	
			count = up->port.fifosize - bytes_in_fifo;
			if (uart_circ_chars_pending(xmit) < count)
				count = uart_circ_chars_pending(xmit);
			...
			{	
				...
				{			
					...
					count	-= tmp;
				//	DEBUG_INTR("count = %d,tmp = %d,xmit->tail = %d\n",count,tmp,xmit->tail);
				}
				else
				{		
					...
					up->port.icount.tx += count;
					count = 0;
				//	DEBUG_INTR("0:count = %d,xmit->tail = %d\n",count,xmit->tail);
				}
...
static int serialexar_setRS485(struct uart_port *port, int turnOn, int turnaroundTime)
{
	...
	DEBUG_INTR("enter serialexar_setRS485\n");
	...
	DEBUG_INTR("leave serialexar_setRS485\n");
	return 0;
}
...
#else
	if(up->port.type == PORT_XR17V35X)
		baud = adv_uart_get_baud_rate(port, termios, old, 0, 921600); 
	else
...
///lipeng
	...
	if(up->port.type == PORT_XR17V35X)
	{
		...
		if(!((baud == 38400) && ((port->flags & UPF_SPD_MASK) == UPF_SPD_CUST)))
		{
			...
			/* if RS485,we set DLD[7] */
			quot_fraction = up->port.unused[0] == SERIAL_MODE_RS485ORRS422S ? ((quot_fraction&0x0f)|0x80) : (quot_fraction&0x0f);
		...
		// set RS485
		up->port.unused[0] == SERIAL_MODE_RS485ORRS422S ? serialexar_setRS485(&up->port, 1, 0) : serialexar_setRS485(&up->port, 0, 0);
