...
#define PCI_DEVICE_ID_ADVANTECH_A003         0xA003
		  ...
		  struct serial_struct *req, int idx)
{
	...
	|| dev->device == PCI_DEVICE_ID_ADVANTECH_A003
	...
	)
	{
		...
		req->reserved_char[0] = (config485 & (0x01 << ((idx % 4)*2))) ? ACR_DTR_RS232 : activeType;
		req->reserved_char[1] |= readw((void *)(remap + XR_17V35X_ID));

		if( (req->reserved_char[1] & 0x88) == 0x88)
		{
			if(config485 & (0x01 << ((idx % 4)*2)))
				req->reserved_char[0] =  ACR_DTR_RS232;
			else if(config485 & (0x01 << ((idx % 4)*2 + 1)))
				req->reserved_char[0] =  SERIAL_MODE_RS485ORRS422S;
			else 
				req->reserved_char[0] =  SERIAL_MODE_RS422M;
		}
		iounmap((void *)remap);
	...
	else // UART_TYPE_AUTO
	{
		...
		if(req->type & 0x10){
			...
			if (pci_resource_flags(cfgdev, ((bar-0x10)/0x04)) & IORESOURCE_MEM) {
				...
				{
					config485_958 = readw((void *)(remap + offset485 + 0x100 + (idx-8)*0x10));
				}
...
static struct pci_serial_quirk pci_serial_quirks[] = {
	...
	{
		.vendor		= PCI_VENDOR_ID_ADVANTECH,
		.device		= PCI_DEVICE_ID_ADVANTECH_A003,
		.subvendor	= PCI_ANY_ID,
		.subdevice	= PCI_ANY_ID,
		.setup		= pci_advantech_setup,
	},
...
static struct pci_device_id adv_serial_pci_tbl[] = {
	...
	{	PCI_VENDOR_ID_ADVANTECH, PCI_DEVICE_ID_ADVANTECH_A002, 
		PCI_ANY_ID, PCI_ANY_ID, 0, 0, 
		pbn_b0_2_921600 },
	{	PCI_VENDOR_ID_ADVANTECH, PCI_DEVICE_ID_ADVANTECH_A003, 
		PCI_ANY_ID, PCI_ANY_ID, 0, 0, 
		pbn_b0_3_921600 },
