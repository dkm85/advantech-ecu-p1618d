 ...
 *
 */
#define ADVANTECH_16C950_VER                    "3.42.7"
#define ADVANTECH_16C950_DATE                   "04/19/2019"
...
//carl add to support PCM-3642I 2015/3/2
#define PCI_DEVICE_ID_ADVANTECH_PCM3642P        0x3642 //PCM-3642P
#define PCI_DEVICE_ID_ADVANTECH_PCMF642P        0xF642 //PCM-3642P
//carl add end
...
//carl add 2015/3/2
#define PCI_SUB_VENDOR_ID_ADVANTECH_PCM3642P    0x3642 //PCM-3642P
//carl add end
...
// support for Exar358's cards
#define PCIE_DEVICE_ID_ADVANTECH_0014			0x0014
#define PCIE_DEVICE_ID_ADVANTECH_0015			0x0015
#define PCIE_DEVICE_ID_ADVANTECH_0016			0x0016
...
struct uart_port_ext uart_ext[64];
static unsigned char serialmode;
static unsigned char reuse;
		  ...
		  struct uart_port *port, int idx)
{
	...
	case PCI_SUB_VENDOR_ID_ADVANTECH_PCM3641P:
		...
		break;
		//carl add 2015/3/2
	case PCI_SUB_VENDOR_ID_ADVANTECH_PCM3642P:
		printk( "PCM-3642P" );
		configType = UART_TYPE_RS232;	
		break;
		//carl add end
	...
	case PCI_VENDOR_ID_ADVANTECH:
		...
		case PCI_DEVICE_ID_ADVANTECH_PCIE952:
			printk( "PCIE952");
			reuse |=  0x01;//this bit means to use new way to calculate baudrate
			reuse |= 0x02; //have DMA
			reuse |= 0x10; //is PCIe952/4/8
			configFunc = 0;
	...
	|| dev->device == PCIE_DEVICE_ID_ADVANTECH_0014
	|| dev->device == PCIE_DEVICE_ID_ADVANTECH_0015
	|| dev->device == PCIE_DEVICE_ID_ADVANTECH_0016)
	{
		activeType = ACR_DTR_ACTIVE_LOW_RS485;
		reuse |= 0x20; //Is XR chip
		printk("Advantech General COM Port Device");
		...
		if(idx<4)
			config485 = readw((void *)(remap + XR_17V35X_MPIOLVL_L));
		else
			config485 = readw((void *)(remap + XR_17V35X_MPIOLVL_H));

		//printk("port485 = 0x%x, len = 0x%x, config485=0x%x############\n",port485,len,config485);
		serialmode = (config485 & (0x01 << ((idx % 4)*2))) ? ACR_DTR_RS232 : activeType;
		reuse |= readw((void *)(remap + XR_17V35X_ID));

		if( (reuse & 0x88) == 0x88)
		{
			if(config485 & (0x01 << ((idx % 4)*2)))
				serialmode =  ACR_DTR_RS232;
			else if(config485 & (0x01 << ((idx % 4)*2 + 1)))
	...
	else // UART_TYPE_AUTO
	{
		...
		if(reuse & 0x10){
			...
			if (pci_resource_flags(cfgdev, ((bar-0x10)/0x04)) & IORESOURCE_MEM) {
				...
				serialmode = (config485_958 & (0x01 << base_idx)) ?
		...
		serialmode = (config485 & (0x01 << (base_idx+idx))) ?
					 activeType : ACR_DTR_RS232;
	}
done:
...
static struct pci_serial_quirk pci_serial_quirks[]  = {
	...
	{//carl add 2015/3/2
		.vendor		= PCI_VENDOR_ID_ADVANTECH,
		.device		= PCI_DEVICE_ID_ADVANTECH_PCM3642P,
		.subvendor	= PCI_SUB_VENDOR_ID_ADVANTECH_PCM3642P,
		.subdevice	= PCI_ANY_ID,
		.setup		= pci_advantech_setup,
	},
	{
		.vendor		= PCI_VENDOR_ID_ADVANTECH,
		.device		= PCI_DEVICE_ID_ADVANTECH_PCMF642P,
		.subvendor	= PCI_SUB_VENDOR_ID_ADVANTECH_PCM3642P,
		.subdevice	= PCI_ANY_ID,
		.setup		= pci_advantech_setup,
	},//carl add end
	...
	{
		.vendor		= PCI_VENDOR_ID_ADVANTECH,
		.device		= PCIE_DEVICE_ID_ADVANTECH_0014,
		.subvendor	= PCI_ANY_ID,
		.subdevice	= PCI_ANY_ID,
		.setup		= pci_advantech_setup,
	},	
	{
		.vendor		= PCI_VENDOR_ID_ADVANTECH,
		.device		= PCIE_DEVICE_ID_ADVANTECH_0015,
		.subvendor	= PCI_ANY_ID,
		.subdevice	= PCI_ANY_ID,
		.setup		= pci_advantech_setup,
	},	
	{
		.vendor		= PCI_VENDOR_ID_ADVANTECH,
		.device		= PCIE_DEVICE_ID_ADVANTECH_0016,
		.subvendor	= PCI_ANY_ID,
		.subdevice	= PCI_ANY_ID,
		.setup		= pci_advantech_setup,
	},		
...
adv_pciserial_init_ports(struct pci_dev *dev, const struct pciserial_board *board)
{
	...
	memset(priv, 0, sizeof(struct serial_private) + sizeof(unsigned int )* nr_ports);
	priv->dev = dev;
	priv->quirk = quirk;
	...
	for (i = 0; i < nr_ports; i++) {
		...
		//printk("serial_type[0] = %d\n",serial_port.unused[0]);
		if (priv->line[i] < 0) {
			printk(KERN_WARNING "Couldn't register serial port %s: %d\n", pci_name(dev), priv->line[i]);
			break;
		}
		uart_ext[priv->line[i]].idx = i;
		uart_ext[priv->line[i]].serial_type = serialmode;
		uart_ext[priv->line[i]].reuse = reuse;
	}
	priv->nr = i;
	return priv;
...
static struct pci_device_id serial_pci_tbl[] = {
	...
	{	PCI_VENDOR_ID_ADVANTECH, PCI_DEVICE_ID_ADVANTECH_PCM3641P, 
		...
		pbn_b0_4_921600 },
		//carl add to support PCM-3642I 8 PORTS RS232
	{	PCI_VENDOR_ID_ADVANTECH, PCI_DEVICE_ID_ADVANTECH_PCM3642P, 
		PCI_SUB_VENDOR_ID_ADVANTECH_PCM3642P, PCI_ANY_ID, 0, 0, 
		pbn_b0_4_921600},
	{	PCI_VENDOR_ID_ADVANTECH, PCI_DEVICE_ID_ADVANTECH_PCMF642P, 
		PCI_SUB_VENDOR_ID_ADVANTECH_PCM3642P, PCI_ANY_ID, 0, 0, 
		pbn_b0_4_921600 },
		//carl add end
	...
	{	PCI_VENDOR_ID_ADVANTECH, PCI_DEVICE_ID_ADVANTECH_A838, 
		PCI_ANY_ID, PCI_ANY_ID, 0, 0, 
		pbn_b0_8_xr_921600 },
	{	PCI_VENDOR_ID_ADVANTECH, PCIE_DEVICE_ID_ADVANTECH_0014, 
		PCI_ANY_ID, PCI_ANY_ID, 0, 0, 
		pbn_b0_2_xr_7812500 },	
	{	PCI_VENDOR_ID_ADVANTECH, PCIE_DEVICE_ID_ADVANTECH_0015, 
		PCI_ANY_ID, PCI_ANY_ID, 0, 0, 
		pbn_b0_4_xr_7812500 },	
	{	PCI_VENDOR_ID_ADVANTECH, PCIE_DEVICE_ID_ADVANTECH_0016, 
		PCI_ANY_ID, PCI_ANY_ID, 0, 0, 
		pbn_b0_8_xr_7812500 },			
	{ 0, }
};
