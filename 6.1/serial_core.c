					...
					const struct ktermios *old_termios)
{
	 ...
	 */
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 12, 0)
	tty_port_set_cts_flow(port, termios->c_cflag & CRTSCTS);
	tty_port_set_check_carrier(port, !(termios->c_cflag & CLOCAL));
#else
...
//jianfeng.dai add: enable DTRDST flow control
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 7, 0)
...
#endif
{
	...
	tmp = retinfo;
#else
	struct serial_struct tmpretinfo;
	struct tty_port *port = &state->port;
	tmp = &tmpretinfo;
#endif
...
static int uart_set_info(struct tty_struct *tty, 
             struct uart_state *state,
			 struct serial_struct __user *newinfo)
{
	struct serial_struct * new_serial;
#if LINUX_VERSION_CODE < KERNEL_VERSION(3, 7, 0)	
	struct serial_struct new_serialinfo;
#endif
	struct uart_port *uport = state->uart_port;
	struct tty_port *port = &state->port;
	...
	if (port->tty) {
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 12, 0)
		/* low_latency removed in 5.12, no-op */
			 ...
			 struct serial_struct __user *newinfo)
{
	 ...
	 */
	mutex_lock(&port->mutex);
	retval = uart_set_info(tty, state, &new_serial);
	mutex_unlock(&port->mutex);
	return retval;
}
#endif
...
static int uart_do_autoconfig(struct tty_struct *tty,struct uart_state *state)
{
	...
	if (tty_port_users(port) == 1) {
		...
		//ret = uart_startup(tty, state, 1); //jianfeng mask
	}
	mutex_unlock(&port->mutex);
	return ret;
}
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 7, 0) &&  LINUX_VERSION_CODE < KERNEL_VERSION(4, 0, 0) 
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 14, 0)
	{
		unsigned long fifo_timeout = uart_fifo_timeout(port);
		char_time = (fifo_timeout > HZ/50) ?
			(fifo_timeout - HZ/50) / port->fifosize : 1;
...
void
adv_uart_configure_port(struct uart_driver *drv, struct uart_state *state,
		    struct uart_port *port)
{
	unsigned int flags;
...
#elif (LINUX_VERSION_CODE < KERNEL_VERSION(3, 20, 0)  || LINUX_VERSION_CODE >= KERNEL_VERSION(4, 4, 0)) 
...
int adv_uart_add_one_port(struct uart_driver *drv, struct uart_port *uport)
{
	...
	adv_uart_configure_port(drv, state, uport);
...
//EXPORT_SYMBOL_GPL(adv_uart_handle_dcd_change);
...
//EXPORT_SYMBOL_GPL(adv_uart_handle_cts_change);
...
void adv_uart_insert_char(struct uart_port *port, unsigned int status,
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 9, 0)
		if ( tty_insert_flip_char(&port->state->port, ch, flag) == 0)
...
EXPORT_SYMBOL_GPL(adv_uart_insert_char);
